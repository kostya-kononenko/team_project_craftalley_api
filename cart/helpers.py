        ...
        try:
            ...
            for cart_item in self.cart_items:
                ...
                self.cost = ((self.calculator.cost_per_delivery * self.number_of_deliveries)
                             + (self.calculator.cost_per_product * self.number_of_products)
                             + self.calculator.fixed_cost)

            return self.cost
        except Exception as e:
            print("Error when trying to getting "
                  "coupon_discounts {0}".format(str(e)))
            return False
...
class CartHelper:
    ...
    def get_delivery_cost(self):
        delivery_helper = DeliveryCostHelper(
    ...
    def get_coupon_discounts(self):
        coupon_helper = CouponHelper(
            cart_total_amount=self.cart_base_total_amount)
        self.discounts["coupons"] = (
            coupon_helper.get_coupon_discounts())

    def get_campaign_discounts(self):
        campaign_helper = CampaignHelper(self.cart_items)
        self.discounts["campaigns"] = (
            campaign_helper.get_campaign_discounts())
            ...
            for discount in self.discounts.get("campaigns", []):
                if discount.discount_type == "Amount":
                    self.campaign_discount_amounts.append(
                        discount.amount.get("amount"))

                if discount.discount_type == "Rate":
                    self.campaign_discount_amounts.append(
                        (self.cart_base_total_amount * discount.amount.get(
                            "rate"))
                        / 100
                    )

            for discount in self.discounts.get("coupons", []):
                self.coupon_discount_amount = (self.cart_base_total_amount
                                               * discount.amount.get("rate")) / 100
        except Exception as e:
            print(
                "Error when trying to calculating "
                "discount amounts {0}".format(str(e))
            )

    def get_total_amount_after_discounts(self):
        if len(self.campaign_discount_amounts) > 0:
            self.campaign_discount_amount = max(
                self.campaign_discount_amounts)
